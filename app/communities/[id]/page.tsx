...
export default function CommunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchCommunity = async (id: string, userId: string) => {
    ...
    const [memberCheck, membersData, eventsData, postsData, likesData] = await Promise.all([
      ...
      supabase.from('community_members').select('*, profile:profiles!community_members_user_id_fkey(id, name, bio_social, avatar_url)').eq('community_id', id).order('joined_at', { ascending: true }).limit(20),
    ...
    if (memberCheck.data) { setIsMember(true); setMemberRole(memberCheck.data.role) }
      ...
      <div className="flex border-b border-white/10 px-4 gap-5">
        {(['feed', 'events', 'members'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab === 'feed' ? `Feed${posts.length > 0 ? ' · ' + posts.length : ''}` : tab === 'events' ? `Events${events.length > 0 ? ' · ' + events.length : ''}` : `Members · ${members.length}`}
          </button>
        ))}
      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <button onClick={() => router.push('/profile/' + profile?.id)}
                            className="text-sm font-semibold text-[#F0EDE6]">
                            {profile?.name || 'Unknown'}
                          </button>
                          <span className="text-[9px] text-white/25">{formatTime(post.created_at)}</span>
                          {isOwn && (
                            <button onClick={() => handleDeletePost(post.id)}
                              className="ml-auto text-[9px] text-white/20 hover:text-red-400">✕</button>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 pt-2 border-t border-white/10">
                      <button
                        ...
                        className={'flex items-center gap-1.5 text-xs font-medium transition-colors ' +
                          (post.liked ? 'text-[#E8B84B]' : 'text-white/30')}
                      >
