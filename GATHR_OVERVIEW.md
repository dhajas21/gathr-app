...
**Community posts:** Text posts visible to all members. Liking a post creates a row in `community_post_likes` and a DB trigger atomically increments `like_count`. Posts support **threaded comments** — replies are stored in `community_post_comments` and loaded lazily on expand. Comment counts are bulk-fetched via a GROUP BY query on page load. Any member can delete their own post/comment; owners and admins can delete any post, comment, or chat message for moderation.
