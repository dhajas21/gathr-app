...
├── home/                 Main feed (events + communities tabs). Header: Gathr wordmark + greeting, city pill, notifications bell. Search bar (tap-through to /search). "Happening Soon" horizontal scroll strip — **tab-aware**: Friends tab filters to events with friend RSVPs; Mine tab filters to the user's own events/RSVPs; other tabs show all soon events. Featured event card (gold pill category tag). Tab bar: Trending / For You / Near Me / Friends / Mine — **Near Me** shows a gold GPS fallback banner (inline "Use GPS" button) when `geoGranted` is false. Event cards with category gradient banners, tag pills, capacity "% full" indicator, bookmark toggle. **Single unified toast system** (`toast: string` state, `showToast(message, duration)` helper) — replaces the old separate city toast and bookmark toast; one bottom-anchored `role="status" aria-live="polite"` element covers all transient feedback.
...
├── communities/[id]/     Community detail — banner (with back + share + settings buttons for owner), info card (name, private lock badge OR unlisted link-icon badge, member count), stats bar (members/posts/events), tabs: Feed (post text+photos, like, comment, load-more), Events (upcoming event list), Members (pending requests panel for owner/admin, member list with "you" label and role badges), Chat (real-time group chat with optimistic updates, delete own messages). CTA bar adapts per state: Join / Request to Join / Cancel Request / Leave+Share / Leave+Create Event (owner/admin only) / chat input. Leave confirmation text is context-aware: private communities say "You'll need to request approval to rejoin"; public/unlisted say "You can rejoin anytime". Feed and posts are gated for both private AND unlisted communities — non-members see a locked state ("Private Community" or "Members Only"). **Non-members always land on the Events tab for private/unlisted communities** so they see content immediately rather than a locked feed. **Event creation is owner/admin only** — regular members cannot create or link events to a community. Owners/admins have two paths: "+ New" (routes to /create?community=id, pre-linking the event) and "+ Add Existing" (modal listing all public events hosted by any community member — host name shown for events the owner didn't create themselves). Linking a member's event uses the `link_event_to_community` SECURITY DEFINER RPC which verifies the caller's community role before updating `events.community_id`, bypassing the events RLS policy that otherwise restricts updates to the event's own host.
├── search/               Universal search — events, communities, people, #tags. Results tabs (All/Events/People/Communities/Tags). Category chip filter row. Pre-search state: recent searches, recently viewed events, "Picked for you" scored recommendations, browse-by-category grid with subcategory expansion (SUBCAT_TO_PARENT resolves 33 subcategories to their top-level DB category). Vibe parser: 55+ synonyms, past-event exclusion, #tag extraction. **localStorage is user-scoped**: recent searches and recently viewed events are stored under keys like `gathr_recent_searches_${userId}` — no cross-user data leakage on shared devices.
...
4. `/setup` detects resumption on load: if `name` AND `interests` are both set in the DB, it prefills all fields, jumps to step 2 (interests — always the missing piece for returning users), and shows a gold "Welcome back" banner. **Google OAuth fix:** Google pre-populates `profiles.name` on signup, so the old `if (data.name)` resume check incorrectly sent brand-new Google users to step 2, skipping name entry. The guard is now `if (data.name && data.interests?.length)`. The setup page also accepts a `?step=N` (3–5) query param — used by the match-profile nudge — to land directly on a specific step. Fresh users always start at step 0.
...
| `event_addresses` | Street addresses for events, gated behind RLS. One row per event (FK → `events.id` ON DELETE CASCADE). `location_address text`. SELECT: host or RSVPed attendee only. INSERT/UPDATE/DELETE: host only. Service role (geocoding edge function) bypasses RLS. Replaced `events.location_address` column which was client-side-gated only. |
...
| `link_event_to_community(p_event_id, p_community_id)` | two UUIDs | Allows a community owner or admin to link any member's public event to the community. Verifies that `auth.uid()` holds `owner` or `admin` role in `community_members` before running `UPDATE events SET community_id = p_community_id WHERE id = p_event_id`. SECURITY DEFINER bypasses the `events` RLS policy that otherwise restricts UPDATE to the event's own host. Used by the "+ Add Existing" modal on the community Events tab. |
...
- Compatibility between two users is computed by `compute_match_score(p_viewer_id, p_candidate_id, p_event_mode text DEFAULT 'social')` — a weighted Postgres function. Weights: shared-interest Jaccard ×50, `vibe` match ×15, `looking_for` overlap ×10 (+5 community-seeker bonus), mutual connections ×10, same city ×5, both early-journey ×5. Only `'social'` mode is implemented; passing any other value raises an exception.
- Scores drive the ranked match list on the event detail page and the mutual dating filter in Paths Crossed
- `get_event_matches(p_event_id uuid, p_viewer_id uuid)` SECURITY DEFINER RPC returns scored, ranked match cards — Gathr+ callers receive identity-enriched cards pre-RSVP; free callers get rows with NULL identity fields (count still populated)

**Phase 0.5 onboarding signals** (collected during setup, after interests):
- `looking_for` (`text[]`, max 3) — what brings you here: `new_to_city`, `activity_partners`, `deepen_friendships`, `life_change_community`, `do_more_stuff`, `curious`. Nullable — NULL scores 0 on that dimension, no error.
- `vibe` (`text`) — event energy preference: `low_key`, `active`, `high_energy`, `mix`. Nullable — NULL skips vibe scoring.
- `offering` (`text`, max 150 chars) — free-text "What's your thing" field; shown on own profile, match cards, and Paths Crossed as a conversation starter
...
- **Connection request hydration on load, pagination, and realtime** — after fetching (both initial load and `loadMore` pagination), `hydrateConnectionStatuses()` batch-queries the `connections` table for all `connection_request` notifications and pre-sets `_accepted` or `_resolved` flags so the Accept/Decline buttons reflect actual DB state across page refreshes and scroll-loads. The realtime INSERT handler also passes each arriving notification through `hydrateConnectionStatuses` before prepending it to state — ensuring newly received notifications never show stale Accept/Decline buttons if the request was already handled on another device. If the query itself fails for any reason, the function falls back to showing buttons (never hides them on error). Notifications with a null `actor_id` are passed through unchanged.
...
- `notifications/page.tsx` — subscribes to INSERT on `notifications` filtered by `user_id`, so new notifications appear without a refresh. Each arriving row is passed through `hydrateConnectionStatuses` before being prepended to state, so connection-request notifications carry the correct `_accepted` / `_resolved` flags even when the request was already handled on another device.
- `host/page.tsx` — creates one filtered channel per host event (`event_id=eq.{id}`) for INSERT and DELETE on `rsvps`. Each channel fires only for that specific event — no client-side guard needed. Previous implementation used a single unfiltered subscription (all RSVPs in the DB) and dropped non-matching payloads client-side; replaced to avoid the unnecessary WebSocket traffic.
- `profile/page.tsx` — subscribes to UPDATE on `connections` filtered by both `requester_id=eq.{uid}` and `addressee_id=eq.{uid}` (two listeners on one channel). When either fires with `status=accepted`, the new connection is appended to state immediately — the connections count in the stats bar updates live without waiting for the 60s stale-refetch threshold.
...
3. Matches a 55+ synonym table covering all 9 categories (`live music / dj / jazz / karaoke → Music`, `coffee / brunch / bar → Food & Drink`, `hike / kayak / skiing → Outdoors & Adventure`, `meditation / pilates / sound bath → Wellness & Mindfulness`, `trivia / pub quiz / game night → Social & Parties`, `conference / summit / founders → Networking`, etc.) → resolves to a top-level category. Multi-word phrases are matched before single-word synonyms so e.g. `"game night"` wins over `"game"` alone.
...
5. **Subcategory → parent resolution** (`SUBCAT_TO_PARENT` map): when the user taps a subcategory tile in the browse grid (e.g. "DJ & Electronic", "Coffee & Brunch", "Hiking & Camping"), the 33-entry map resolves it to its top-level DB category before the `.eq('category', ...)` filter fires — events are stored with top-level categories only, so this makes subcategory browsing return real results instead of zero
...
- `components/icons.tsx` — centralized SVG icon barrel. Named exports: `MapPinIcon`, `CalendarIcon`, `SearchIcon`, `BookmarkIcon` (with `filled` prop), `BellIcon`, `UserIcon`, `PeopleIcon`, `InfoCircleIcon`, `ChevronDownIcon`, `XIcon`, `MessageIcon`. All share a base `Svg` wrapper with `strokeWidth 1.75`, round caps/joins, and `size`/`className`/`strokeWidth` props. Always import from here — never write one-off inline SVGs for icon-sized UI elements.
- `components/EmptyState.tsx` — shared empty state component. Props: `icon` (ReactNode, typically from the icon barrel), `headline` (rendered in `font-editorial-italic`), `body` (optional), `action` (`{ label, onClick, variant: 'primary' | 'secondary' }`, optional). Icon wrapper has `empty-float` class (gentle bounce keyframe). Used across bookmarks, notifications, search, communities list, community detail (posts + chat), and profile (events + connections). Never write one-off inline empty state blocks in page files.
...
| Shared SVG icon barrel | `components/icons.tsx` — `MapPinIcon`, `CalendarIcon`, `SearchIcon`, `BookmarkIcon`, `BellIcon`, `UserIcon`, `PeopleIcon`, `InfoCircleIcon`, `ChevronDownIcon`, `XIcon`, `MessageIcon` |
| Shared empty state | `components/EmptyState.tsx` — `icon`, `headline`, `body`, `action` props; `empty-float` bounce on icon wrapper |
...
- **UI polish primitives applied**:
  ...
  - `empty-float` keyframe is a global CSS class for a gentle icon bounce — now used via `components/EmptyState` (applied to the icon wrapper) across all pages
...
- **RSVP-gated address reveal**: Street address is gated behind RSVP at both the UI and the database. `location_address` was moved out of the `events` table into `event_addresses` — a separate table with RLS: SELECT restricted to the event host or a user who has RSVPed (enforced server-side, not just in the UI). The `geocode-event` edge function reads from `event_addresses` using the service-role client (which bypasses RLS). Non-RSVPed viewers see venue name only + "RSVP to unlock full address"; RSVPed users and the host see the full address and the Maps button. Calendar exports also use only the `locationAddress` state variable (which is empty for non-authorized users).
...
- **Connection direction awareness on public profile** (`app/profile/[id]/page.tsx`): The connect button on a public profile previously treated any `pending` connection row (in either direction) as "yours to withdraw" — if the other user had already sent you a request, clicking "Withdraw Request" would silently delete their request instead. Root cause: the connection query uses an OR filter matching both `(requester=you, addressee=them)` and `(requester=them, addressee=you)`, but the handler never checked which direction the row was. Fix: added `connectionDirection` state (`'sent' | 'received' | null`) set from `connectionRes.data.requester_id === userId`. Withdrawal is now only possible when `direction === 'sent'`. When `direction === 'received'`, the button renders "Respond to Request →" and taps through to `/notifications` where the proper Accept/Decline UI lives.
- **Realtime notification hydration** (`app/notifications/page.tsx`): Notifications pushed via the realtime INSERT channel were prepended to state as raw rows — no `hydrateConnectionStatuses` call. A notification arriving while another device had already accepted/declined would show stale Accept/Decline buttons until the page was refreshed. Fix: the realtime handler is now `async`, awaits `hydrateConnectionStatuses([payload.new], userId)`, and prepends the enriched row.
- **Event cover image not loading** (`app/events/[id]/page.tsx`): The `fetchEvent` Supabase select query listed every column individually but omitted `cover_url`. The hero banner was therefore always `undefined` and the image never rendered. Fix: added `cover_url` to the select string.
- **Welcome modal scroll + pull-to-refresh bleed** (`app/home/page.tsx`): The inner welcome bottom-sheet div lacked `max-h-[85vh] overflow-y-auto`, causing it to expand beyond the viewport without internal scrolling. Additionally, touch events on the modal were propagating to the home feed's pull-to-refresh layer, triggering a refresh while scrolling the sheet. Fix: added `max-h-[85vh] overflow-y-auto` to the inner div and `onTouchMove={e => e.stopPropagation()}` to block event bubbling.
- **Friends tab not updating after connection accept** (`app/home/page.tsx`): The home page had no realtime subscription for connection state changes. After accepting a connection request on the notifications page, the Friends tab showed "No connections yet" until a manual refresh. Fix: added a dedicated `postgres_changes` realtime channel (`home-connections-{userId}`) that watches `UPDATE` events on the `connections` table filtered to rows where `requester_id` or `addressee_id` equals the current user. On receiving an `accepted` status payload, it calls `fetchAll` to reload Friends-tab data immediately.
- **Own profile showing offering** (`app/profile/page.tsx`): The `offering` field ("What's your thing") was fetched via `select('*')` but never rendered on the own-profile view. Added an italicised quote display below the bio — visible when `profile.offering` is non-empty.
- **Dating toggle silent error** (`app/settings/page.tsx`): When the `toggle-dating-intent` edge function returned a non-2xx (e.g. 429 Too Many Requests), the UI correctly reverted the toggle but showed no error message. Fix: added `datingError` state; on `!res.ok` the handler reads the response body JSON, shows "Too many toggles — try again tomorrow." for 429 or the body's `error` field otherwise. Network-level failures show "Could not reach server — check your connection." Error rendered inline below the dating toggle.
...
- **Phase 0.5 onboarding signals** (migration `20260516000000`, `app/setup/page.tsx`): Two new steps added to setup after interests. `looking_for text[]` (max 3, valid values: new_to_city / activity_partners / deepen_friendships / life_change_community / do_more_stuff / curious) and `vibe text` (low_key / active / high_energy / mix). Both nullable — legacy users without them score 0 on those dimensions gracefully. Also added `offering text` (max 150 chars) — a "What's your thing" free-text field shown on own profile, match cards, and Paths Crossed. Check constraints, GIN/btree indexes, and column comments all in the migration.
...
- **Activity page: empty-state SVG rgba stroke** (`app/notifications/page.tsx`): Bell icon used hardcoded `stroke="rgba(126,200,126,0.45)"` — invisible on the cream background in light mode. Replaced with `stroke="currentColor" className="text-[#7EC87E]/45"`.
- **Activity page: missing notification type icons** (`app/notifications/page.tsx`): `wave`, `achievement`, `community_event`, and `survey_prompt` notification types all fell through to the generic bell icon in `getTypeIconSvg`. Each now has a dedicated SVG — wave (power/ring icon), achievement (star polygon), community_event (calendar), survey_prompt (document with lines).
- **Messages inbox: community chat preview showed raw protocol text** (`app/messages/page.tsx`): When the last message in a community chat was an image or file upload, the preview rendered the raw internal string `[image]https://...` or `[file:filename]https://...`. Fix: preview now checks for the `[image]` and `[file:]` prefixes and substitutes `📷 Photo` / `📎 File` respectively.
- **Messages inbox: three hardcoded rgba SVG strokes** (`app/messages/page.tsx`): Person-icon fallbacks in the connection request card and empty-state connection list used `stroke="rgba(126,200,126,0.5)"` (invisible in light mode); swipe-tip SVG used `stroke="rgba(232,184,75,0.7)"`. All three replaced with `stroke="currentColor"` + appropriate `text-[color]/opacity` class.
- **Messages inbox: handleAccept missing pending guard** (`app/messages/page.tsx`): `handleAccept` called `update({ status: 'accepted' }).eq('id', connectionId)` without a `.eq('status', 'pending')` guard. Added to match the notifications page pattern and prevent acting on an already-accepted or withdrawn row.
- **DM thread: typing indicator not cleared on send** (`app/messages/[id]/page.tsx`): When the user sent a message, `presenceChannelRef.current.track({ typing: false })` was not called — the typing indicator persisted on the recipient's screen for up to 2 seconds after the message arrived. Fix: clear the typing timeout and broadcast `typing: false` immediately inside `handleSend` before inserting the message row.
- **Match-profile nudge** (`app/profile/page.tsx`, `app/setup/page.tsx`): Own-profile page shows a dismissable gold-tinted card below the bio/offering when `profile.looking_for` is null or empty and `gathr_match_profile_nudge_seen` is not set in localStorage. "Complete →" routes to `/setup?step=3`. Dismiss (×) writes `gathr_match_profile_nudge_seen = '1'` and hides the card permanently. `app/setup/page.tsx` updated to read `?step` query param (valid range 3–5): if present and the user already has a name, skips the normal resume-to-step-2 and lands directly on the specified step.
